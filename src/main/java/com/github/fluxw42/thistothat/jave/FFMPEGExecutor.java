 ...
 */
package com.github.fluxw42.thistothat.jave;

import com.github.fluxw42.thistothat.util.OSType;
...
class FFMPEGExecutor {
	...
	public FFMPEGExecutor(String ffmpegExecutablePath) {
		this.ffmpegExecutablePath = ffmpegExecutablePath == null ?
				getFFMPEGExecutablePath() : ffmpegExecutablePath;
	}
	...
	private static String getFFMPEGExecutablePath() {
		...
		switch (osType) {
			case LINUX: {
				return new File("./native/linux/ffmpeg").getAbsolutePath();
			}

			case WINDOWS: {
				return new File(".\\native\\windows\\ffmpeg.exe").getAbsolutePath();
			}

			default: {
				throw new IllegalStateException("Could not get ffmpeg path: OS type [" + osType + "] not supported.");
			}
		}
	}
